...
import '../page/wallet/MercadoPagoScreen.dart';
...
class RidePaymentController extends GetxController {
  ...
  Future<void> getArgument() async {
    ...
    if (argumentData != null) {
      ...
      if (type.value == "rental") {
        rentalBookingData.value = argumentData['rentalBookingModel'];
        calculateRentalTotalAmount();
      } else {
  ...
  void calculateRentalTotalAmount() {
    taxAmount = "0.0".obs;
  ...
  Future<void> setAmount() async {
    ...
    if (type.value == "rental") {
      Map<String, dynamic> requestBody = {
        ...
        "id_user": rentalBookingData.value.user!.id,
        "id_driver": rentalBookingData.value.driver!.id,
        "id_payment": value,
      ...
      await API
          .handleApiRequest(
              request: () => http.post(Uri.parse(API.completeRequest), headers: API.headers, body: jsonEncode(requestBody)),
              showLoader: false)
          .then(
        (value) {
          if (value != null) {
  ...
  Future<void> changePaymentMethod() async {
    ...
    Map<String, dynamic> requestBody = {
      "booking_id": type.value == "rental" ? rentalBookingData.value.id : bookingModel.value.id,
      "booking_type": type.value,
      "id_payment": value,
    };
    await API
        .handleApiRequest(
            request: () => http.post(Uri.parse(API.changeBookingPaymentMethod), body: jsonEncode(requestBody), headers: API.headers),
            showLoader: false)
        .then((value) {
      if (value != null) {
        if (value['success'] == "Failed" || value['success'] == "ailed") {
          ...
          Get.back(result: true);
          ShowToastDialog.showToast("Payment Method Changed Successfully!!");
        }
      }
    });
  }
    ...
    await API.handleApiRequest(request: () => http.get(Uri.parse(API.paymentSetting), headers: API.headers), showLoader: false).then(
      (value) {
        if (value != null) {
          ...
          if (type.value == "rental") {
            selectedPaymentMethod.value = Constant.getGatewayValue(
              key: rentalBookingData.value.paymentMethod.toString(),
              property: "libelle",
              model: paymentSettingModel.value,
            ).toString();
          } else {
            selectedPaymentMethod.value = Constant.getGatewayValue(
              key: bookingModel.value.paymentMethod.toString(),
              property: "libelle",
              model: paymentSettingModel.value,
            ).toString();
          }
